...
import PropertyBuildingModal from '@/components/modal/propertyBuilding'

import { getPropertyBuildList, delBuild } from "@/api";
...
export default function SearchPage() {
  ...
  const getDataList = (data) => {
    getPropertyBuildList(data).then((res) => {
  ...
  const tableTop = (
    <Row justify="space-between" align="center" gutter={80}>
      <Col style={{ lineHeight: "32px" }}>楼栋查询</Col>
      <Col>
  ...
  // 删除档案信息
  const deleteBuild = (info) => {
    delBuild(info.id).then((res) => {
      const { msg, code } = res;
      if (code === 200) {
        message.success(msg);
        getDataList(pageData);
      ...
      <Spin spinning={load}>
        <div className="top-form">
          <Form layout="inline" form={searchForm}>
            <Form.Item name="buildName">
              <Input placeholder="输入楼栋名称" />
            </Form.Item>
