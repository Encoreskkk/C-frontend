...
import PropertyServeModal from '@/components/modal/propertyServe'

import { getPropertyServeList, delServe } from "@/api";
...
export default function SearchPage() {
  ...
  const getDataList = (data) => {
    getPropertyServeList(data).then((res) => {
  ...
  const tableTop = (
    <Row justify="space-between" align="center" gutter={80}>
      <Col style={{ lineHeight: "32px" }}>服务查询</Col>
      <Col>
  ...
  // 删除档案信息
  const deleteServe = (info) => {
    delServe(info.id).then((res) => {
      const { msg, code } = res;
      if (code === 200) {
        message.success(msg);
        getDataList(pageData);
